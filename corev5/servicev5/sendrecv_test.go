...
func TestReadMessageSuccess(t *testing.T) {
	...
	m, n, err := svc.readMessage(messagev5.CONNECT, 62)

	require.NoError(t, err, "Error decoding messagev5.")

	require.Equal(t, len(msgBytes), n, "Error decoding messagev5.")

	msg := m.(*messagev5.ConnectMessage)

	require.Equal(t, messagev5.QosAtLeastOnce, msg.WillQos(), "Incorrect Will QoS")
	...
	require.Equal(t, "send me home", string(msg.WillMessage()), "Incorrect will messagev5 value.")
...
func TestReadMessageError(t *testing.T) {
	msgBytes := []byte{
		byte(messagev5.RESERVED << 4), // <--- WRONG
		60,
		0, // Length MSB (0)
		4, // Length LSB (4)
		'M', 'Q', 'T', 'T',
	...
	_, _, err := svc.readMessage(messagev5.CONNECT, 62)
...
func TestReadMessageError2(t *testing.T) {
	...
	_, _, err := svc.readMessage(messagev5.CONNECT, 64)
...
//	require.NoError(t, err)
//
//	require.Equal(t, msgBytes, dst, "error decoding messagev5.")
//}
