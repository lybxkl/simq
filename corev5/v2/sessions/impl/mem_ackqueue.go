...
type ackqueue struct {
	...
	ping sessions.Ackmsg
	ring []sessions.Ackmsg
	// 查看是否有相同的数据包ID在队列中
	emap map[uint16]int64

	ackdone []sessions.Ackmsg
...
func newMemAckQueue(n int) *ackqueue {
	...
	return &ackqueue{
		...
		head:    0,
		tail:    0,
		ring:    make([]sessions.Ackmsg, m),
		emap:    make(map[uint16]int64, m),
		ackdone: make([]sessions.Ackmsg, 0),
	}
}
func newAckqueue(n int) sessions.Ackqueue {
	m := int64(n)
	if !powerOfTwo64(m) {
		m = roundUpPowerOfTwo64(m)
	}
...
//返回已完成ack循环的消息列表。
func (this *ackqueue) Acked() []sessions.Ackmsg {
	this.mu.Lock()
	defer this.mu.Unlock()
...
func (this *ackqueue) removeHead() error {
	...
	// set this to empty sessions.Ackmsg{} to ensure GC will collect the buffer
	//将此设置为empty sessions.Ackmsg{}，以确保GC将收集缓冲区
	this.ring[this.head] = sessions.Ackmsg{}
...
func (this *ackqueue) grow() {
	...
	newsize := this.size << 1
	newmask := newsize - 1
	newring := make([]sessions.Ackmsg, newsize)
