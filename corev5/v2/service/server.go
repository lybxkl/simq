...
type Server struct {
	...
	// 增强认证允许的方法
	authPlusAllows map[string]authplus.AuthPlus
...
func (this *Server) InitStore() {
	...
	default:
		this.SessionStore = memImpl.NewMemSessionStore()
		this.MessageStore = memImpl.NewMemMessageStore()
	}
...
func (this *Server) handleConnection(c io.Closer) (svc *service, err error) {
	...
	resp := messagev52.NewConnackMessage()
	...
	if err != nil {
		if cerr, ok := err.(messagev52.ReasonCode); ok {
	...
	if svconf.RedirectOpen { // 重定向
		...
		if svconf.RedirectIsForEver {
			dis.SetReasonCode(messagev52.ServerHasMoved)
		} else {
			dis.SetReasonCode(messagev52.UseOtherServers)
		}
	...
	resp.SetReasonCode(messagev52.Success)
...
func (this *Server) auth(conn net.Conn, resp *messagev52.ConnackMessage, req *messagev52.ConnectMessage) error {
	// 增强认证
	...
	if len(authMethod) > 0 {
		...
		if !ok {
			dis := messagev52.NewDisconnectMessage()
			dis.SetReasonCode(messagev52.InvalidAuthenticationMethod)
	...
	AC:
		...
		if err != nil {
			dis := messagev52.NewDisconnectMessage()
			dis.SetReasonCode(messagev52.UnAuthorized)
			dis.SetReasonStr([]byte(err.Error()))
			return writeMessage(conn, dis)
		}
		if continueAuth {
			au := messagev52.NewAuthMessage()
			au.SetReasonCode(messagev52.ContinueAuthentication)
			...
			switch msg.Type() {
			case messagev52.DISCONNECT: // 增强认证过程中断开连接
				return errors.New("disconnect in auth")
			case messagev52.AUTH:
				auMsg := msg.(*messagev52.AuthMessage)
				if !reflect.DeepEqual(auMsg.AuthMethod(), authMethod) {
					ds := messagev52.NewDisconnectMessage()
					ds.SetReasonCode(messagev52.InvalidAuthenticationMethod)
		...
		if err := this.authMgr.Authenticate(string(req.Username()), string(req.Password())); err != nil {
			//登陆失败日志，断开连接
			resp.SetReasonCode(messagev52.UserNameOrPasswordIsIncorrect)
			resp.SetSessionPresent(false)
...
func (this *Server) checkConfiguration() error {
	...
	this.configOnce.Do(func() {
		...
		this.authMgr = auth.NewDefaultAuth()

		auPlus := make(map[string]authplus.AuthPlus)
		for _, s := range this.AuthPlusProvider {
			ma := authplus.NewDefaultAuth()

			auPlus[s] = ma
		}
		this.authPlusAllows = auPlus

		this.sessMgr = sessions_impl.NewMemProvider()

		this.topicsMgr = topics_impl.NewMemProvider()
...
func (this *Server) getSession(svc *service, req *messagev52.ConnectMessage, resp *messagev52.ConnackMessage) error {
