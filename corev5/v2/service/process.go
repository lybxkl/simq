...
func (this *service) sendBeyondQuota() (int, error) {
	...
	dis.SetReasonCode(messagev52.BeyondQuota)
	return this.sendByConn(dis)
}
func (this *service) sendTooManyMessages() (int, error) {
	dis := messagev52.NewDisconnectMessage()
	dis.SetReasonCode(messagev52.TooManyMessages)
	return this.sendByConn(dis)
}

// sendByConn 直接从conn连接发送数据
func (this *service) sendByConn(msg messagev52.Message) (int, error) {
	b := make([]byte, msg.Len())
	_, err := msg.Encode(b)
	if err != nil {
		return 0, err
...
func (this *service) processIncoming(msg messagev52.Message) error {
	var err error = nil
	...
	case *messagev52.PubrecMessage:
		if msg.ReasonCode() > messagev52.QosFailure {
			this.sign.AddQuota()
		}
		...
		resp := messagev52.NewPubrelMessage()
		resp.SetPacketId(msg.PacketId())
		_, err = this.writeMessage(resp)

	case *messagev52.PubrelMessage:
		...
		resp := messagev52.NewPubcompMessage()
		resp.SetPacketId(msg.PacketId())
		_, err = this.writeMessage(resp)

	case *messagev52.PubcompMessage:
		this.sign.AddQuota() // 增加配额
	...
	case *messagev52.UnsubackMessage:
	...
	case *messagev52.PingreqMessage:
		...
		resp := messagev52.NewPingrespMessage()
		_, err = this.writeMessage(resp)
...
func (this *service) processAcked(ackq sessions.Ackqueue) {
	for _, ackmsg := range ackq.Acked() {
		...
		if msg.Type() != messagev52.PINGREQ {
		...
		if ack.Type() == messagev52.PINGRESP {
		...
		case messagev52.PUBREL:
			...
			//发布了，让我们把它发布给其他订阅者吧。
			if err = this.onPublish(msg.(*messagev52.PublishMessage)); err != nil {
				logger.Logger.Errorf("(%s) Error processing ack'ed %s messagev5: %v", this.cid(), ackmsg.Mtype, err)
			}

		case messagev52.PUBACK, messagev52.PUBCOMP, messagev52.SUBACK, messagev52.UNSUBACK:
...
// 入消息的主题别名处理 第一阶段 验证
func (this *service) topicAliceIn(msg *messagev52.PublishMessage) error {
...
func (this *service) processPublish(msg *messagev52.PublishMessage) error {
	if err := this.topicAliceIn(msg); err != nil {
		return err
	}
	switch msg.QoS() {
	case messagev52.QosExactlyOnce:
		this.sess.Pub2in().Wait(msg, nil)

		resp := messagev52.NewPubrecMessage()
		resp.SetPacketId(msg.PacketId())

		_, err := this.writeMessage(resp)
		return err

	case messagev52.QosAtLeastOnce:
		resp := messagev52.NewPubackMessage()
		resp.SetPacketId(msg.PacketId())
...
// For SUBSCRIBE messagev5, we should add subscriber, then send back SUBACK
func (this *service) processSubscribe(msg *messagev52.SubscribeMessage) error {
	resp := messagev52.NewSubackMessage()
	resp.SetPacketId(msg.PacketId())
	...
	for _, t := range tps {
		...
		if this.conFig.Broker.CloseShareSub && len(t) > 6 && reflect.DeepEqual(t[:6], []byte{'$', 's', 'h', 'a', 'r', 'e'}) {
			...
			//}
			unSupport = true
			tmpResCode = append(tmpResCode, messagev52.UnsupportedSharedSubscriptions.Value())
		} else {
			tmpResCode = append(tmpResCode, messagev52.UnspecifiedError.Value())
		}
	}
	switch {
	...
	for i, t := range tps {
		...
		//订阅要停止，就放手吧。
		if retainHandling == messagev52.NoSendRetain {
			continue
		} else if retainHandling == messagev52.CanSendRetain {
			_ = this.topicsMgr.Retained(t, &this.rmsgs)
			logger.Logger.Debugf("(%s) topic = %s, retained count = %d", this.cid(), t, len(this.rmsgs))
		} else if retainHandling == messagev52.NoExistSubSendRetain {
	...
	logger.Logger.Infof("客户端：%s，订阅主题：%s，qos：%d，retained count = %d", this.cid(), tps, qos, len(this.rmsgs))
...
func (this *service) processUnsubscribe(msg *messagev52.UnsubscribeMessage) error {
	topics := msg.Topics()
	...
	resp := messagev52.NewUnsubackMessage()
	resp.SetPacketId(msg.PacketId())
	resp.AddReasonCode(messagev52.Success.Value())
...
func (this *service) processToCluster(topics [][]byte, msg messagev52.Message) {
	if !this.clusterOpen {
		return
	}
	tag := 0
	...
	if tag > 0 { // 确实是有共享主题，才发送到集群其它节点
		...
		case messagev52.SUBSCRIBE:
			...
			msgTo = sub
		case messagev52.UNSUBSCRIBE:
			sub := messagev52.NewUnsubscribeMessage()
			for i := 0; i < len(shareTp); i++ {
...
//主题，并将消息发布到订阅方列表。
func (this *service) onPublish(msg1 *messagev52.PublishMessage) error {
	if msg1.Retain() {
	...
	tmpMsg := messagev52.NewPublishMessage() // 必须重新弄一个，防止被下面改动qos引起bug
	_, _ = tmpMsg.Decode(b)
...
func (this *service) sendShareToCluster(msg1 *messagev52.PublishMessage) {
	if !this.clusterOpen {
		// 没开集群，就只要发送到当前节点下就行
		// 发送当前主题的所有共享组
...
// 发送到集群其它节点去
func (this *service) sendCluster(message messagev52.Message) {
	if !this.clusterOpen {
		return
	}
	submit(func() {
		colong.SendMsgToCluster(message, "", "", func(message messagev52.Message) {

		}, func(name string, message messagev52.Message) {
...
// 集群节点发来的普通消息
func (this *service) ClusterInToPub(msg1 *messagev52.PublishMessage) error {
...
func (this *service) ClusterInToPubShare(msg1 *messagev52.PublishMessage, shareName string, onlyShare bool) error {
	if !this.clusterBelong {
		return nil
	}
	if msg1.Retain() {
...
// 集群节点发来的系统主题消息
func (this *service) ClusterInToPubSys(msg1 *messagev52.PublishMessage) error {
	if !this.clusterBelong {
		return nil
	}
	if msg1.Retain() {
...
func (this *service) pubFn(msg *messagev52.PublishMessage, shareName string, onlyShare bool) error {
	var (
		subs []interface{}
		qoss []topics.Sub
	)
...
// 发送当前主题所有共享组
func (this *service) pubFnPlus(msg *messagev52.PublishMessage) error {
	var (
		subs []interface{}
		qoss []topics.Sub
	)
...
// 发送系统消息
func (this *service) pubFnSys(msg *messagev52.PublishMessage) error {
	var (
		subs []interface{}
		qoss []topics.Sub
	)
